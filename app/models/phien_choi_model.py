...
class PhienChoiModel:
    ...
    def create(
        ...
        gia_gio_ap_dung: float | None,
        sale_percent_ap_dung: float = 0.0,
    ) -> int:
        now = datetime.now().isoformat(timespec="seconds")
        cur = self._conn.execute(
            """INSERT INTO phien_choi
               (ban_id, nhan_vien_id, gio_bat_dau, gio_ket_thuc, gia_gio_ap_dung, sale_percent_ap_dung)
               VALUES (?, ?, ?, NULL, ?, ?)""",
            (ban_id, nhan_vien_id, now, gia_gio_ap_dung, float(sale_percent_ap_dung)),
        )
        self._conn.commit()
    ...
    def list_active(self) -> list[sqlite3.Row]:
        cur = self._conn.execute(
            """SELECT p.id, p.ban_id, p.nhan_vien_id, p.gio_bat_dau,
                      p.gia_gio_ap_dung, p.sale_percent_ap_dung,
                      b.ten_ban, b.gia_gio
               FROM phien_choi p
